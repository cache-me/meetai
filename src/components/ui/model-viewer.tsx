...
// Separate components for each model type to avoid conditional hook calls
const GLTFModel: FC<{
  url: string;
...
// Model loader component that conditionally renders the appropriate loader
...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  }, [content, camera, autoFrame, fadeIn, initPitch, initYaw, pivot, onLoaded]);
  ...
  if (!content) {
    return (
      <>
        <ModelLoader url={url} onLoad={handleModelLoad} />
        <group ref={outer}>
          <group ref={inner} />
        </group>
      </>
